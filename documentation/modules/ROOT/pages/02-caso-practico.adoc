...
NOTE: `Podman auto-update` requiere la generación de `service units`. Para facilitar el laboratorio, se ha creado la `service unit` `tetris` mediante el comando `podman generate systemd --new tetris`, que crea el contenedor `tetris` con la imagen `{registry}/tetris:latest` y la label `io.containers.autoupdate=registry`.


NOTE: Para más información sobre la `service unit` `tetris`, **puedes preguntar a nuestro Staff**.
...
NOTE: Si quieres lanzar `podman auto-update` manualmente, puedes ejecutarlo directamente en la terminal con el comando `podman auto-update`.
