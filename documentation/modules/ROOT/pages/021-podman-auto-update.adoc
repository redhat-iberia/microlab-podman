...
En el navegador de archivos, seleccionamos a la izquierda `Carpeta Personal`, y a la derecha `tetris` -> `v1`, y seleccionamos el archivo `Containerfile`. Como nombre de imagen, en el campo `Image Name`, pondremos `{registry}/tetris:v1`.
...
Repetiremos los pasos realizados para el depliege del Tetris, pero usando en el Build la carpeta `v2`. Pondremos también el tag de `latest`, y haremos un push de la imagen.
